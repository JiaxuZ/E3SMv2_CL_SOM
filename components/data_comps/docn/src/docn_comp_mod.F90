...
module docn_comp_mod
  ...
  integer(IN)   :: kswnet,klwup,klwdn,ksen,klat,kmelth,ksnow,krofi,kevap,krain,kmeltw
  integer(IN)   :: kh,kqbot,krofl,kbergw,kbergh
...
CONTAINS
  ...
  subroutine docn_comp_init(Eclock, x2o, o2x, &
    ...
    !!added
    !kbergh = mct_aVect_indexRA(x2o,'Fioi_bergh')
    !kbergw = mct_aVect_indexRA(x2o,'Fioi_bergw')
    kevap  = mct_aVect_indexRA(x2o,'Foxx_evap')
    krain  = mct_aVect_indexRA(x2o,'Faxa_rain')
    kmeltw = mct_aVect_indexRA(x2o,'Fioi_meltw')
    krofl  = mct_aVect_indexRA(x2o,'Foxx_rofl')
    ...
    case('SOM_MPAS')
       ...
       if (firstcall) then
          ...
          do n = 1,lsize
             if (imask(n) /= 0) then
                !--- pull out h from av for resuse below ---
                hn = avstrm%rAttr(kh,n) !+ (x2o%rAttr(ksnow,n) + x2o%rAttr(krofi,n) + & 
                     !x2o%rAttr(kevap,n) + x2o%rAttr(krain,n) + x2o%rAttr(kmeltw,n) + x2o%rAttr(krofl, n))*dt/rhosw
                !--- compute new temp ---
                o2x%rAttr(kt,n) = somtp(n) + &
